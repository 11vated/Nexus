...
class ChatSession:
    ...
    def _setup_compaction(self) -> None:
        """Initialize the context compaction pipeline.

        Prevents context overflow in long conversations by compacting
        old messages when the context window is approaching capacity.
        """
        try:
            ...
            # Determine model window size from config
            model_key = "14b" if "14b" in self.config.coding_model else "7b"
            model_window = MODEL_CONTEXT_WINDOWS.get(model_key, MODEL_CONTEXT_WINDOWS["default"])

            self._compaction_pipeline = ContextCompactionPipeline(
                model_context_window=model_window,
                config=CompactionConfig(
                    model_context_window=model_window,
                    fast_model=self.config.compaction_model,
                    ollama_url=self.config.ollama_url,
                ),
            )
            self._compaction_enabled = self.config.compaction_enabled
            logger.info(
                "Context compaction initialized (window=%d, model=%s)",
                model_window, self.config.compaction_model,
            )
        except ImportError as exc:
            logger.warning("Context compaction not available: %s", exc)
            self._compaction_enabled = False

    def _setup_persistence(self) -> None:
        """Initialize persistence manager for auto-save/load of state."""
        try:
            from nexus.cognitive.persistence import PersistenceManager
            self._persistence = PersistenceManager(workspace=self.workspace)

            # Load previously persisted state
            state = self._persistence.load_all()

            # Restore knowledge if available
            if state.get("knowledge") and self._cognitive:
                try:
                    self._cognitive.knowledge.restore(state["knowledge"])
                    logger.info("Restored knowledge from persistence")
                except Exception as exc:
                    logger.warning("Failed to restore knowledge: %s", exc)

            # Restore memory if available
            if state.get("memory") and self._cognitive:
                try:
                    self._cognitive.memory.restore(state["memory"])
                    logger.info("Restored memory from persistence")
                except Exception as exc:
                    logger.warning("Failed to restore memory: %s", exc)

            logger.info("Persistence manager initialized")
        except Exception as exc:
            logger.warning("Persistence not available: %s", exc)
            self._persistence = None

    def _auto_save_if_needed(self) -> None:
        """Check if autosave is due and perform it."""
        if not self._persistence or not self._cognitive:
            return
    ...
    async def _maybe_compact_context(self, user_message: str) -> None:
        """Check if context needs compaction and run the pipeline if so.

        Called at the start of each turn to prevent context overflow.
        """
        if not self._compaction_enabled or not self._compaction_pipeline:
            return

        # Calculate current history character count
        history_chars = sum(len(m.get("content", "")) for m in self.history)

        if not self._compaction_pipeline.should_compact(history_chars):
            return

        # Gather context for compaction
        goal = self.history[0].get("content", "") if self.history and self.history[0].get("role") == "user" else ""
        plan_steps = []
        assumptions = []
        user_prefs = []

        # Extract from cognitive layer if available
        if self._cognitive:
            plan_steps = [s.get("title", "") for s in self._cognitive.get_plan()]

        # Run compaction
        try:
            result = await self._compaction_pipeline.compact(
                messages=self.history,
                goal=goal,
                plan_steps=plan_steps,
                assumptions=assumptions,
                user_preferences=user_prefs,
                session_stats={"turns": self.turn_count, "tool_calls": self._tool_call_count},
            )
            ...
            logger.info(
                ...
                result.chars_before, result.chars_after, result.messages_removed,
            )
        except Exception as exc:
    ...
    def get_compaction_stats(self) -> Dict[str, Any]:
        ...
        if not self._compaction_pipeline:
            return {"enabled": False}
        stats = self._compaction_pipeline.stats
        stats["enabled"] = self._compaction_enabled
        stats["current_history_chars"] = sum(
            len(m.get("content", "")) for m in self.history
        )
        return stats
    ...
    def save_session(self) -> Dict[str, bool]:
        ...
        Archives the current conversation if it has messages.

        Returns:
            Dict of component name → success status.
        """
        if not self._persistence:
            logger.warning("No persistence manager; cannot save session")
            return {}
        ...
        if self._cognitive:
            try:
                knowledge_data = self._cognitive.knowledge.to_dict()
                memory_data = self._cognitive.memory.to_dict()
                cognitive_data = self._cognitive.to_dict()

                results = self._persistence.save_all(
        ...
        if self.history and self.session_id:
            try:
                ...
                results["conversation_archived"] = True
            except Exception as exc:
                logger.warning("Failed to archive conversation: %s", exc)
                results["conversation_archived"] = False
    ...
    def load_conversation(self, session_id: str) -> bool:
        ...
        archive = self._persistence.load_conversation(session_id)
        if archive and "messages" in archive:
            self.history = list(archive["messages"])
            logger.info("Loaded conversation %s (%d messages)", session_id, len(self.history))
            return True
        return False

    def list_archived_conversations(self) -> List[Dict[str, Any]]:
        """List all archived conversations."""
        if not self._persistence:
            return []
        return self._persistence.list_conversations()

    def search_archived_conversations(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        """Search archived conversations."""
        if not self._persistence:
            return []
        return self._persistence.search_conversations(query, limit)
    ...
    async def send(self, user_message: str) -> AsyncIterator[ChatEvent]:
        ...
        # === Context Compaction: Check and compact if needed ===
        await self._maybe_compact_context(user_message)

        # === Persistence: Auto-save if interval elapsed ===
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        if self._compaction_pipeline:
            result["compaction"] = self.get_compaction_stats()
